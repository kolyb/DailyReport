...
namespace DailyReport.DataAccess.Repositories
{
    public class PlanRepository : IRepository<Plan>
    {
        ...
        public PlanRepository(DailyReportContext db)
        {
            _db = db;
            _db.ChangeTracker.QueryTrackingBehavior = QueryTrackingBehavior.NoTracking;
        }

        public async Task CreateAsync(Plan item)
        {
            _db.Plans.Add(item);
            await _db.SaveChangesAsync();
        }

        public async Task DeleteAsync(Plan item)
        {
            _db.Plans.Remove(item);
            await _db.SaveChangesAsync();
        }
        ...
        {
            var result = await _db.Plans.FindAsync(id);
            if (result == null)
            {
                //
                throw new Exception();
        ...
        public async Task UpdateAsync(Plan item)
        {
